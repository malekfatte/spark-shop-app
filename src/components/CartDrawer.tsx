          ...
          {items.length === 0 ? (
            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-3 p-3 rounded-lg bg-secondary/30">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-heading font-medium text-sm truncate">{item.product.node.title}</h4>
                        <p className="text-xs text-muted-foreground">{item.selectedOptions.map(o => o.value).join(' • ')}</p>
                        <p className="font-semibold text-sm text-primary">${parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t">
                ...
                <Button onClick={handleCheckout} className="w-full glow-red" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
